...
import { securityScoreSchema, type RuleOutcome, type Section } from '@/types/analysis';
 ...
 *
 * The score is rendered with its methodology, its excluded rules, and its band caps, because a
 * number without those is not auditable. The disclaimer text comes from the payload, so the API,
 * the UI, and the Markdown report cannot drift into saying different things.
 *
 * This is the only score WebLens displays anywhere.
 */
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            No posture score was produced for this scan. A score is only emitted when enough rules
            could be evaluated for the ratio to mean something.
          </CardDescription>
        </CardHeader>
      </Card>
    );
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div
            ...
            role="img"
            aria-label={`${score.percentage}% of applicable points`}
          >
            <div
      ...
      {score.excluded_rules.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Excluded from both the numerator and the denominator, so the ratio stays auditable.
            </CardDescription>
          </CardHeader>
          <CardContent>
